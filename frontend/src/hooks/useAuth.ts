...
export function useAuth(): AuthContextType {
  ...
  const doRefreshToken = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const data = await api.post<Tokens>("/auth/refresh", { refresh_token: tokens.refresh_token });
      setTokens(data);
  ...
  const register = async (email: string, username: string, password: string, full_name?: string) => {
    await api.post<User>("/auth/register", { email, username, password, full_name });
  ...
  const refreshUser = async () => {
    if (tokens) {
      try {
        const userData = await api.get<User>("/auth/me", tokens.access_token);
        setUser(userData);
      } catch (err) {
  ...
  return { user, tokens, isLoading, login, register, logout, refreshUser, refreshToken: doRefreshToken };
}
